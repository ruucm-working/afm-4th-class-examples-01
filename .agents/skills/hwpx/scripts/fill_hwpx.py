...
사용법:
    ...
    python fill_hwpx.py analyze form.hwpx

    # 2) 채우기 (values.json: {"성명": "홍길동", "연락처": "010-1234-5678"})
    python fill_hwpx.py fill form.hwpx output.hwpx --values values.json

    # 2-1) 좌표 직접 지정 (라벨 매칭이 안 통하는 복잡한 표 폴백)
    python fill_hwpx.py fill form.hwpx output.hwpx \
        --cells cells.json   # [{"table":0,"row":2,"col":1,"value":"텍스트"}]

    # 3) 문구 교체 — run 경계로 쪼개진 텍스트도 잡음 (내용 수정)
    python fill_hwpx.py replace doc.hwpx out.hwpx --map map.json

    # 4) 표 행 추가 — 기존 행 복제라서 스타일/너비/테두리 보존 (내용 추가)
    python fill_hwpx.py add-row doc.hwpx out.hwpx --table 1 --rows rows.json

    # 5) 검증 (값이 실제로 들어갔는지 + 비변경 엔트리 바이트 동일성)
    python fill_hwpx.py verify output.hwpx --values values.json --original form.hwpx
