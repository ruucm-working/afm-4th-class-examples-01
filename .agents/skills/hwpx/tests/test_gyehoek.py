...
사용법: python tests/test_gyehoek.py
"""
...
def gen(**kw):
    ...
    if have_lxml:
        ...
        c = subprocess.run([sys.executable, str(ROOT / "scripts/fill_hwpx.py"),
                            "check", str(out), "--strict"], capture_output=True, text=True, encoding="utf-8", errors="replace").returncode
    return out, x, v, c
...
check("플래그 없음 → 차단(2)", hook("python scripts/gyehoek.py -o a.hwpx") == 2)
check("제목O목차O → 통과(0)",
      hook("python scripts/gyehoek.py --title '제목' --toc -o a.hwpx") == 0)
check("제목X목차X → 통과(0)",
      hook("python scripts/gyehoek.py --no-title --no-toc -o a.hwpx") == 0)
check("목차 결정 없음 → 차단(2)",
      hook("python scripts/gyehoek.py --title '제목' -o a.hwpx") == 2)
check("계획서 아님 → 통과(0)", hook("python scripts/gonmun.py --sample") == 0)
...
check("test_gyehoek.py 실행 → 통과(0)", hook("python tests/test_gyehoek.py") == 0)
